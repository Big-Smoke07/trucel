...
client.on('messageCreate', async (message) => {
  ...
  const manualVoteMatch = message.content.trim().match(/^\?(addupvotes|adddownvotes|removeupvoted|removeupvotes|removedownvotes)\s+<@!?(\d+)>\s+(\d+)$/i);
  if (manualVoteMatch) {
    const caller = await message.guild.members.fetch(message.author.id).catch(() => null);
    if (!caller?.roles.cache.has(config.managerRoleId)) {
      await message.reply('You do not have the required role to adjust vote totals.');
      return;
    }
    const [, action, memberId, amountText] = manualVoteMatch;
    const normalizedAction = action.toLowerCase();
    const direction = normalizedAction.includes('upvote') ? 'up' : 'down';
    const requestedAmount = Number(amountText);
    if (!Number.isSafeInteger(requestedAmount) || requestedAmount < 1 || requestedAmount > 100_000) {
      await message.reply('Use a whole number from 1 to 100000.');
      return;
    }
    const amount = normalizedAction.startsWith('add') ? requestedAmount : -requestedAmount;
    let adjustedAmount = 0;
    await runSerially(async () => {
      const totals = ensureMember(memberId);
      const previousTotal = totals[direction];
      totals[direction] = Math.max(0, previousTotal + amount);
      adjustedAmount = Math.abs(totals[direction] - previousTotal);
      await saveState();
    });
    const limitedNote = adjustedAmount !== requestedAmount ? ' (limited to the available total)' : '';
    await message.reply(`${amount > 0 ? 'Added' : 'Removed'} **${adjustedAmount} ${direction}vote${adjustedAmount === 1 ? '' : 's'}** ${amount > 0 ? 'for' : 'from'} <@${memberId}>.${limitedNote}`);
    return;
  }
