...
client.on('messageCreate', async (message) => {
  ...
  const manualVoteMatch = message.content.trim().match(/^!(addupvotes|adddownvotes|removeupvoted|removeupvotes|removedownvotes)\s+<@!?(\d+)>$/i);
  if (manualVoteMatch) {
    if (!message.member?.roles.cache.has(config.managerRoleId)) {
    ...
    const amount = normalizedAction.startsWith('add') ? 1 : -1;
    await runSerially(async () => {
      const totals = ensureMember(memberId);
      totals[direction] = Math.max(0, totals[direction] + amount);
      await saveState();
    });
    await message.reply(`${amount > 0 ? 'Added' : 'Removed'} **1 ${direction}vote** ${amount > 0 ? 'for' : 'from'} <@${memberId}>.`);
    return;
  }
